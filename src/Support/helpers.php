...
if (!function_exists('arrHasSubstrateKey')) {
    /**
     * Determine if an item exists in an array using "dot" notation,
     * but allowing dots in key names.
     *
     * @return bool
     */
    function arrHasSubstrateKey(array $array, string $key)
    {
        ...
        while (!empty($parts)) {
            // Try the longest compound key first
            for ($length = count($parts); $length > 0; $length--) {
                $compoundKey = implode('.', array_slice($parts, 0, $length));

                if (array_key_exists($compoundKey, $subArray)) {
                    $subArray = $subArray[$compoundKey];
...
if (!function_exists('arrGetSubstrateKey')) {
    /**
    ...
    function arrGetSubstrateKey(array $array, string $key, mixed $default = null)
    {
        ...
        while (!empty($parts)) {
            ...
            // If no key matches, return the default value
            return $default;
        }

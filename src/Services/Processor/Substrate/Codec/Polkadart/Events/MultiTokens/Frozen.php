...
class Frozen implements PolkadartEvent
{
    ...
    public static function fromChain(array $data): self
    {
        ...
        $self->collectionId = Arr::get($data, 'event.MultiTokens.Frozen.collection_id');
        $self->freezeType = is_string($freezeType = Arr::get($data, 'event.MultiTokens.Frozen.freeze_type')) ? $freezeType : array_key_first($freezeType);
    ...
    protected static function getTokenId(array $data, string $freezeType): ?string
    {
        ...
        return $freezeType === 'Token'
            ? Arr::get($data, 'event.MultiTokens.Frozen.freeze_type.Token.token_id') ?? Arr::get($data, 'event.MultiTokens.Frozen.freeze_type.Token')
            : Arr::get($data, 'event.MultiTokens.Frozen.freeze_type.TokenAccount.token_id');
    }
    ...
    protected static function getAccount(array $data, string $freezeType): ?string
    {
        ...
        return $freezeType === 'CollectionAccount'
            ? Arr::get($data, 'event.MultiTokens.Frozen.freeze_type.CollectionAccount')
            : Arr::get($data, 'event.MultiTokens.Frozen.freeze_type.TokenAccount.account_id');
    }
}

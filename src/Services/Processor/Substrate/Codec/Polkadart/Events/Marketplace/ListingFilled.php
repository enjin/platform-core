...
class ListingFilled extends Event implements PolkadartEvent
{
    ...
    public static function fromChain(array $data): self
    {
        ...
        $self->buyer = Account::parseAccount($self->getValue($data, 1));
        $self->price = $self->getValue($data, 2);
        $self->amountFilled = $self->getValue($data, 3);
        $self->amountRemaining = $self->getValue($data, 4);
        $self->protocolFee = $self->getValue($data, 5);
        $self->royalty = $self->getValue($data, 6);
